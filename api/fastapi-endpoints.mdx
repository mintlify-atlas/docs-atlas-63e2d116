...
The health endpoint is exempt from authentication and used for container health checks.
...
<Note>
  The `/health` endpoint bypasses authentication middleware (see main.py:34)
</Note>
...
All endpoints (except `/health`, `/docs`, and `/openapi.json`) are protected by the authentication middleware. See [Authentication](/api/authentication) for details.
